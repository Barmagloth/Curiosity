...
[Coarse representation] — грубое начальное приближение
       |
       v
[Функция информативности p(x)] — определяет, где уточнять
       |
       v
...
## Компонент 1: Функция информативности p(x)
...
- Если здоров: p = residual-only (нулевые потери на clean данных)
...
- Halo инициализируется нулём, ограничен по энергии
- При отключении текущего уровня — валидный откат к предыдущему
...
- Morton и block-sparse layouts: **неэффективны** (overhead превышает выигрыш)
