...
| `exp10f_packed_lookup/` | Packed tiles + alternative lookup (hash/direct) | P0 (0.9b2) | 🔓 Открыт |
...
│         B's storage с hash/direct lookup вместо binary search.
│         Кандидаты: cuckoo hash, tile-level direct index, pre-built neighbour lists.
│         Kill criteria: те же (>20% overhead vs grid per pattern class)
│
├── 0.9b:  end-to-end pipeline (после exp10f)
...
- A (bitset) — лучший execution layout (быстрее grid на 27-31%), но не экономит VRAM
- B's packed storage — перспективен по VRAM (-30% при sparse), убит binary search lookup
- exp10f решит: можно ли спасти B's storage заменой lookup?

**Выход P0:** фиксированный layout. Либо A (grid+bitset, быстрее, но не экономит VRAM),
либо B-with-better-lookup (если exp10f покажет выигрыш и по времени и по VRAM).
