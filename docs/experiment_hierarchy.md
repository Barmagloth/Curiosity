...
| `exp10g_dual_benchmark/` | Dual-mode benchmark: manual stencil (layout cost) vs conv2d (operator cost). Resolves D's Contour B. | P0 (0.9b3) | 🔓 Открыт |
...
│         Manual stencil (layout cost) vs conv2d (operator cost).
│         Resolves D's Contour B failure.
...
**Текущий статус P0:**
- Grid — рабочий baseline
- A (bitset) — operational default (быстрее grid на 27-31%), не экономит VRAM
- D (exp10f) — passes Contour A, fails Contour B (peak VRAM from conv2d workspace). Pending resolution via exp10g.
- E (exp10f) — archived as contingency (resurrection triggers documented)
- exp10g (dual-mode benchmark) решит: manual stencil (layout cost) vs conv2d (operator cost) → resolves D's Contour B

**Выход P0:** фиксированный layout. A — operational default.
