...
**Примечание Phase 3 (22.03.2026):** Четыре эксперимента Phase 3 завершены. Exp14 (anchors): grid divergence=0 (PASS), graph/tree >0.20 (FAIL) — local update вносит structural drift на нерегулярных пространствах. Exp15 (LCA-distance): Spearman r<0.3 во всех пространствах — дерево не семантично. Exp15b (bushes): silhouette>0.4 (кластеры есть), ARI<0.21 (не стабильны). Exp16 (C-pre): gap>1.0, sil>0.3 все пространства — Track C UNFREEZE.
