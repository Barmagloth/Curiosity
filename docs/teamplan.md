...
### Пост-Phase 4 / Трек исследований

| Эксперимент | Описание | Зависимости |
|-------------|----------|-------------|
...
| **Streaming budget control (B+C)** | Плавное управление бюджетом в streaming (сейчас только бинарные go/stop). Два механизма: **(B)** L0-informed allocation — формула институционального неравенства: $$W_{cluster} = N_{units} \times (1 - ECR)^{\gamma}$$, γ ≥ 2. Квадратичная аллокация из термодинамики StrictnessTracker (GREEN ~90%, YELLOW ~72%, RED ~42% номинала). **(C)** Adaptive redistribution — неиспользованный бюджет кластера перетекает к следующим (forward carry). γ валидируется в sweep: γ ∈ {1.0, 1.5, 2.0, 2.5, 3.0, 4.0} вместе с Governor EMA. | Phase 4 pipeline + L0 zones |
| **Exp Governor-sweep** | Валидация Governor EMA + streaming B+C на эмулированных конфигурациях железа. Sweep: 3 режима pipeline (batch, frozen reuse, streaming) × 3 эмулированных hardware profile (low/mid/high budget) × 4 пространства × 20 seeds. Метрики: PSNR, wall time, reject rate, waste exhaustion rate, compliance. Kill criteria: (1) batch+reuse: Governor EMA улучшает compliance vs no-governor; (2) streaming B+C: PSNR ≥ equal-allocation baseline. Зависимости: Governor EMA restoration + streaming B+C. | Governor EMA + B+C |
