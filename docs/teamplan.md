...
| **Streaming budget control (B+C)** | Плавное управление бюджетом в streaming (сейчас только бинарные go/stop). Два механизма: **(B)** L0-informed allocation — бюджет по кластерам пропорционально ожидаемой полезности (GREEN → больше, RED → меньше), не просто по размеру; **(C)** Adaptive redistribution — неиспользованный бюджет кластера перетекает к следующим (forward carry). Тестировать в sweep вместе с Governor EMA. | Phase 4 pipeline + L0 zones |
