...
| **Streaming budget control (B+C)** | Smooth budget control for streaming mode (currently only binary go/stop). Two mechanisms: **(B)** L0-informed allocation — Institutional Inequality Formula: $$W_{cluster} = N_{units} \times (1 - ECR)^{\gamma}$$, γ ≥ 2. Quadratic allocation derived from StrictnessTracker thermodynamics (GREEN ~90%, YELLOW ~72%, RED ~42% of nominal). **(C)** Adaptive redistribution — unspent cluster budget flows to subsequent clusters (forward carry). γ to be validated in sweep: γ ∈ {1.0, 1.5, 2.0, 2.5, 3.0, 4.0} alongside Governor EMA. | Phase 4 pipeline + L0 zones |
